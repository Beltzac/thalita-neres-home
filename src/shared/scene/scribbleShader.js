...
export function initScribbleShader({ container, strokeCount = 150, canvasClass = 'scribbleLayer' }) {
  ...
  let lastOpenness = 0;
  let pixelRatio = 1;
  let centerClientX = null;
  let centerClientY = null;
  ...
  function draw() {
    ...
    const centerX = centerClientX === null ? 0 : ((centerClientX - canvasRect.left) / canvasRect.width) * 2 - 1;
    const centerY = centerClientY === null ? 0 : 1 - ((centerClientY - canvasRect.top) / canvasRect.height) * 2;
  ...
  return { setOpenness, setCenter, canvas };
}
